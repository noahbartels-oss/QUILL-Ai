...
import { Feather, Github, Loader2, AlertCircle, CheckCircle2 } from "lucide-react";
...
export default function RegisterPage() {
  ...
  if (success) {
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-violet-50 via-white to-indigo-50">
        <div className="w-full max-w-md text-center">
          <CheckCircle2 className="h-16 w-16 text-green-500 mx-auto mb-4" />
          <h2 className="text-2xl font-bold mb-2">{t("success")}</h2>
          <Button asChild variant="gradient" className="mt-4">
            <Link href={`/${locale}/auth/login`}>{t("login_link")}</Link>
          </Button>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-violet-50 via-white to-indigo-50">
      <div className="w-full max-w-md">
        ...
        {/* Card */}
        <div className="bg-white rounded-2xl border shadow-xl shadow-violet-500/5 p-8">
          {/* OAuth Buttons */}
